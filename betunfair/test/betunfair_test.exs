defmodule BetUnfairTest do
  ...
  # Tests the correct cancellation of the market after the freeze of a market and the cancel of a bet
  test "market_freeze_cancel" do
    assert {:ok,_} = BetUnfair.clean("testdb")
    assert {:ok,_} = BetUnfair.start_link("testdb")
    assert {:ok,u1} = BetUnfair.user_create("u1","Tristan")
    assert {:ok,u2} = BetUnfair.user_create("u2","Manuel")
    ...
    assert is_ok(BetUnfair.user_deposit(u3,2000))
    assert {:ok,m1} = BetUnfair.market_create("rmw","Real Madrid wins")
    assert {:ok,b1} = BetUnfair.bet_back(u1,m1,300,150)
    assert {:ok,b2} = BetUnfair.bet_lay(u2,m1,100,150)
    assert {:ok,b3} = BetUnfair.bet_lay(u3,m1,100,150)
    assert {:ok, %{balance: 1700, id: "u1", name: "Tristan"}} = BetUnfair.user_get(u1)
    assert {:ok, %{balance: 1900, id: "u2", name: "Manuel"}} = BetUnfair.user_get(u2)
    assert {:ok, %{balance: 1900, id: "u3", name: "Alonso"}} = BetUnfair.user_get(u3)
    assert is_ok(BetUnfair.market_match(m1))
    assert {:ok, %{bet_id: b1, bet_type: :back, market_id: m1, user_id: u1, odds: 150, original_stake: 300, remaining_stake: 0, matched_bets: [b3,b2], status: :active}} = BetUnfair.bet_get(b1)
    assert {:ok, %{bet_id: b2, bet_type: :lay, market_id: m1, user_id: u2, odds: 150, original_stake: 100, remaining_stake: 0, matched_bets: [b1], status: :active}} = BetUnfair.bet_get(b2)
    assert {:ok, %{bet_id: b3, bet_type: :lay, market_id: m1, user_id: u3, odds: 150, original_stake: 100, remaining_stake: 50, matched_bets: [b1], status: :active}} = BetUnfair.bet_get(b3)
    assert is_ok(BetUnfair.bet_cancel(b1))
    assert {:ok, %{bet_id: b1, bet_type: :back, market_id: m1, user_id: u1, odds: 150, original_stake: 300, remaining_stake: 0, matched_bets: [b3,b2], status: :cancelled}} = BetUnfair.bet_get(b1)
    assert is_ok(BetUnfair.market_freeze(m1))
    assert {:ok, %{balance: 1700, id: "u1", name: "Tristan"}} = BetUnfair.user_get(u1)
    assert {:ok, %{balance: 1900, id: "u2", name: "Manuel"}} = BetUnfair.user_get(u2)
    assert {:ok, %{balance: 1900, id: "u3", name: "Alonso"}} = BetUnfair.user_get(u3)
  ...
  # Tests the correct settle of a market after cancelling a bet with remaining stake
  test "bet_cancel_settle" do
    assert {:ok,_} = BetUnfair.clean("testdb")
    assert {:ok,_} = BetUnfair.start_link("testdb")
    assert {:ok,u1} = BetUnfair.user_create("u1","Tristan")
    assert {:ok,u2} = BetUnfair.user_create("u2","Manuel")
    ...
    assert {:ok, %{bet_id: b3, bet_type: :lay, market_id: m1, user_id: u3, odds: 150, original_stake: 100, remaining_stake: 50, matched_bets: [b1], status: :active}} = BetUnfair.bet_get(b3)
    assert is_ok(BetUnfair.bet_cancel(b3))
    assert {:ok, %{bet_id: b3, bet_type: :lay, market_id: m1, user_id: u3, odds: 150, original_stake: 100, remaining_stake: 50, matched_bets: [b1], status: :cancelled}} = BetUnfair.bet_get(b3)
    assert is_ok(BetUnfair.market_settle(m1, false))
    assert {:ok, %{balance: 1700, id: "u1", name: "Tristan"}} = BetUnfair.user_get(u1)
    assert {:ok, %{balance: 2200, id: "u2", name: "Manuel"}} = BetUnfair.user_get(u2)
    assert {:ok, %{balance: 2100, id: "u3", name: "Alonso"}} = BetUnfair.user_get(u3)
  end
  ...
  test "settle_with_cancelled_bets" do
    ...
    assert is_ok(BetUnfair.market_freeze(m1))
    assert is_ok(BetUnfair.market_settle(m1, true))
    assert {:ok, %{balance: 2100, id: "u1", name: "Tristan"}} = BetUnfair.user_get(u1)
    assert {:ok, %{balance: 1900, id: "u2", name: "Manuel"}} = BetUnfair.user_get(u2)
  end

  # Tests the correct cancellation of a bet after having matched partially with others
  test "settle_with_cancelled_bets_2" do
    assert {:ok,_} = BetUnfair.clean("testdb")
    assert {:ok,_} = BetUnfair.start_link("testdb")
    assert {:ok,u1} = BetUnfair.user_create("u1","Tristan")
    assert {:ok,u2} = BetUnfair.user_create("u2","Manuel")
    ...
    assert {:ok, %{balance: 1900, id: "u3", name: "Alonso"}} = BetUnfair.user_get(u3)
    assert is_ok(BetUnfair.market_settle(m1, true))
    assert {:ok, %{balance: 2150, id: "u1", name: "Tristan"}} = BetUnfair.user_get(u1)
    assert {:ok, %{balance: 1900, id: "u2", name: "Manuel"}} = BetUnfair.user_get(u2)
    assert {:ok, %{balance: 1950, id: "u3", name: "Alonso"}} = BetUnfair.user_get(u3)
  end
  ...
  test "bet_match_settle_false" do
    ...
    assert {:ok, %{balance: 41488, id: "u2", name: "Manuel"}} = BetUnfair.user_get(u2)
  end
  ...
  test "bet_match_freeze_settle" do
    ...
    assert is_ok(BetUnfair.market_match(m1))
    assert is_ok(BetUnfair.market_freeze(m1))
    assert {:ok, %{bet_id: a, bet_type: :back, market_id: m1, user_id: u1, odds: 300, original_stake: 2000, remaining_stake: 0, matched_bets: [], status: :active}} = BetUnfair.bet_get(a)
    assert {:ok, %{bet_id: b, bet_type: :back, market_id: m1, user_id: u1, odds: 200, original_stake: 1400, remaining_stake: 0, matched_bets: [], status: :active}} = BetUnfair.bet_get(b)
    assert {:ok, %{bet_id: c, bet_type: :back, market_id: m1, user_id: u1, odds: 153, original_stake: 500, remaining_stake: 311, matched_bets: [g], status: :active}} = BetUnfair.bet_get(c)
    ...
    assert {:ok, %{bet_id: e, bet_type: :lay, market_id: m1, user_id: u2, odds: 110, original_stake: 40000, remaining_stake: 0, matched_bets: [], status: :active}} = BetUnfair.bet_get(e)
